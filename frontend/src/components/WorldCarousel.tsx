...
const WorldCarousel: React.FC = () => {
    ...
    const [sliderRef, instanceRef] = useKeenSlider<HTMLDivElement>({
        ...
        // breakpoints: { // Örnek responsive ayar
        //   '(max-width: 640px)': {
    ...
    // Effect: Kullanıcının mevcut oylarını çekme 
    useEffect(() => {
        const fetchMyVotes = async () => {
            ...
            try {
                ...
                } else { setError("Oylarınız yüklenirken bir sorun oluştu."); }
                setMyVotes(new Set());
            } finally { setIsVotesLoading(false); }
        };
        const token = localStorage.getItem('authToken');
        if (token) { fetchMyVotes(); }
        else { console.log("Kullanıcı girişi yapılmamış, oylar çekilmiyor."); }
    }, []);
    ...
    const handleLikeClick = async (participantId: number) => {
        ...
        try {
            ...
            alert(`Hata: ${error.response?.data?.detail || 'Oy verme işlemi sırasında bir sorun oluştu.'}`);
            setParticipants(prevParticipants =>
                prevParticipants.map(p =>
    ...
    return (
        ...
        <div className="px-2 sm:px-4 py-4 max-w-4xl mx-auto"> 
            <h2 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-5 text-center text-gray-100 font-minecraft">Minecraft Worlds Tournament</h2>
                                ...
                                <div className="keen-slider__slide" key={participant.id}>
                                    <div className="bg-gradient-to-br from-gray-700 to-gray-900 p-3 sm:p-4 rounded-lg h-full flex flex-col items-center">
                                        <video
                                            ...
                                            controls
                                            className="w-full aspect-video mb-3 sm:mb-4 rounded-md shadow"
                                        >
...
// --- Ok (Arrow) Bileşeni (Responsive iyileştirmelerle) ---
